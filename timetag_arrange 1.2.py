    ...
    if end != False:
        ...
        try:
            ...
            try:
                ...
                try:
                    find_target = re.findall(r'{}(.*)'.format('</a><span dir="auto" class="style-scope yt-formatted-string">'), text)[0]
                    print('sucess.')
                except:
                    print('error in ', text)
...
def reading(identify_by = 'a class', #html語法中，a class是超連結的寫法，所以搜尋這句話有沒有a class判斷是否要執行動作
            text_start = '</a><span dir="auto" class="style-scope yt-formatted-string"> ', #a class跟樣式宣告完後面就是要抓取的文字了，作為預設搜尋起點
